...
Now includes job matching, gap analysis, and tailored resume generation
"""
import sys
import os
import pandas as pd
...
from src.rag.profile_rag import build_or_refresh_profile_index  # ✅ NEW: RAG support
from src.crews.job_match_crew import evaluate_job  # ✅ NEW: Job matching
from src.crews.resume_builder_crew import generate_tailored_resume  # ✅ NEW: Tailored resumes
from src.crews.gap_analyzer_crew import analyze_gaps_for_learning  # ✅ NEW: Gap analysis

def run_h1b_job_finder(generate_resumes=False, match_threshold=0.65):
    """
    ...
    Args:
        generate_resumes: Generate tailored resumes for top matches (default: False)
        match_threshold: Minimum match score for resume generation (default: 0.65)
    """
    print("=" * 70)
    ...
    # ✅ STEP 3: Job matching against your resume (NEW)
    ...
    if generate_resumes and matched_jobs:
        ...
        for i, job in enumerate(matched_jobs, 1):
            ...
            try:
                resume_result = generate_tailored_resume(job['description'], {
                ...
                resume_filename = f"h1b_job_{i}_{job['company'].replace(' ', '_')[:20]}_{job['match_score']:.0%}.docx"
    ...
    if matched_jobs:
        ...
        report_df = df[['title', 'company', 'location', 'source', 
                       'h1b_eligible', 'eligibility_reason', 'match_score',
                       'url', 'resume_path']].head(20)
    ...
    if len(raw_jobs) > 0:
        print(f"H1B exclusion rate:        {((len(raw_jobs)-len(h1b_jobs))/len(raw_jobs)*100):>6.1f}%")
        print(f"Match rate (top jobs):     {((len(matched_jobs)/len(h1b_jobs))*100):>6.1f}%")
    print("=" * 70)

def run_h1b_job_finder_streamlit(keywords, location, num_pages=3, use_ai=True, match_threshold=0.65):
    """
    ...
    Args:
        ...
        match_threshold: Minimum match score (default: 0.65)
    
    Returns:
        dict: {
            'total_jobs': int,
            'h1b_jobs': list of dicts,
            'matched_jobs': list of dicts with match_score,
